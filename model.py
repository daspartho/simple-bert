...
class BertEmbedding(nn.Module):
    def __init__(
        ...
        self.position = nn.Embedding(
            num_embeddings=max_len, embedding_dim=d_model, padding_idx=pad_idx
        )
        self.segment = nn.Embedding(
            num_embeddings=n_segment, embedding_dim=d_model, padding_idx=pad_idx
        )
...
class BertModel(nn.Module):
    def __init__(
        ...
        self.embedding = BertEmbedding(
            vocab_size, max_len, n_segment, d_model, dropout, pad_idx
        )
...
class BertPreTrain(nn.Module):
    ...
    def forward(self, token_ids, position_ids, segment_ids):
        ...
        next_sentence_prediction_logits = self.next_sentence_prediction(
            hidden_states[:, 0]
        )
...
if __name__ == "__main__":
    ...
    next_sentence_prediction, masked_lm = bert_pre_train(
        token_ids, position_ids, segment_ids
    )
    ...
    next_sentence_prediction = bert_next_sentence_prediction(
        token_ids, position_ids, segment_ids
    )
    ...
    sequence_classification = bert_sequence_classification(
        token_ids, position_ids, segment_ids
    )
    ...
    token_classification = bert_token_classification(
        token_ids, position_ids, segment_ids
    )
